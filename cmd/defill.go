...
func newDeFillCommand() *cobra.Command {
	f := deFillFlags{
		...
		from: []string{audio.Group320Mp3, audio.GroupFLAC},
	}
	...
	command := cobra.Command{
		...
		Run: func(_ *cobra.Command, args []string) {
			f.from = lo.FlatMap(f.from, func(item string, _ int) []string {
				return strings.Split(item, ",")
			})
			from := lo.SliceToMap(f.from, func(item string) (string, struct{}) {
				return item, struct{}{}
			})
			from[audio.GroupFLAC] = struct{}{}

			f.groups = lo.FlatMap(f.groups, func(item string, _ int) []string {
				return strings.Split(item, ",")
			})
			bitrates := make(map[string]struct{}, len(f.groups))
			for _, group := range f.groups {
				if _, ok := from[group]; ok {
					continue
				}
				bitrates[group] = struct{}{}
			}
			...
			for basename, availableBitrates := range checkMap {
				for keep := range from {
					// Check if there is any matching bitrate to keep.
					if _, ok := availableBitrates[keep]; ok {
	...
	command.Flags().StringSliceVarP(&f.from, "from", "f", f.from, "Bitrate groups to keep (FLAC is always included)")
